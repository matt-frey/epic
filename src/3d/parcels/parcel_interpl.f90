...
module parcel_interpl
    ...
    contains
        ...
        subroutine grid2par(vel, vortend, vgrad, add)
            ...
            !$omp parallel default(shared)
            !$omp do private(n, l, is, js, ks, weights, vor, dvdx, dwdz)
            do n = 1, n_parcels
            ...
            vsum = sum(parcels%volume(1:n_parcels))
            vortend(1, 1:n_parcels) = vortend(1, 1:n_parcels) - sum(vortend(1, 1:n_parcels) * parcels%volume(1:n_parcels)) / vsum
            vortend(2, 1:n_parcels) = vortend(2, 1:n_parcels) - sum(vortend(2, 1:n_parcels) * parcels%volume(1:n_parcels)) / vsum
            vortend(3, 1:n_parcels) = vortend(3, 1:n_parcels) - sum(vortend(3, 1:n_parcels) * parcels%volume(1:n_parcels)) / vsum
